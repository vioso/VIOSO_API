...
VWB_ERROR VWB_Warper_base::ReadIniFile( std::filesystem::path configFile, char const* szChannelName ) {
	if( !configFile.empty() ) {
		Default( *this );
	#if defined( WIN32 )
		static char const* libExt = ".dll";
	#elif defined( __APPLE__ )
		// no eye provider
		static char const* libExt = "";
	#else
		static char const* libExt = ".so";
	#endif // defined( WIN32 )
		...
		if( configFile.has_extension() && ( configFile.extension() == ".xml" || configFile.extension() == ".XML" ) ) {
			...
			FILE* f = nullptr;
		#ifdef _WIN32
			// On Windows, use _wfopen with wide strings
			...
			if( element = root->FirstChildElement( "eyePointProvider" ) ) {
				VWBUtil::copy( eyeProvider, (char const*)MkPath( (char8_t const*)element->GetText(), libExt, m_configPath ).u8string().c_str() );
			}
			...
			if( m_bUTF8 )
				VWBUtil::copy( eyeProvider, (char const*)MkPath( (char8_t const*)sDef.c_str(), libExt, m_configPath ).u8string().c_str() );
			else
				VWBUtil::copy( eyeProvider, MkPath( sDef, libExt, m_configPath ).string() );
