...
VWBmap< HGLRC, MyWindow > g_windows;
...
int InitGL( MyWindow& wnd )										// All Setup For OpenGL Goes Here
{
	...
	#if 0
	RECT rWnd;
	GetWindowRect( wnd.hWnd, &rWnd );
	DWORD dwStyle = GetWindowLong( wnd.hWnd, GWL_STYLE );
	DWORD dwExStyle = GetWindowLong( wnd.hWnd, GWL_EXSTYLE );
	...
	BOOL status = wglChoosePixelFormatARB( wnd.hDC, pixelAttribs, NULL, 1, &pixelFormatID, &numFormats );
...
BOOL CreateGLWindow( MyWindow& wnd, char* title, int posX, int posY, int width, int height, int bits )
{
	...
	if( !GetClassInfo( g_hInstance, "OpenGL", &wc ) )
	{
					...
					int			nCmdShow )			// Window Show State
{
	...
	for( size_t i = 0; i != set.size(); i++ )
	{
		...
		try {
			w = g_windows.emplace( wnd.hRC, std::make_shared<VWBX<MyWindow>>( wnd, _T( "" ), nullptr, s_configFile, set[i]->header.name ) ).first->second;
		}
		catch( VWB_ERROR )
		{
			return FALSE;
	...
	while( !done )									// Loop That Runs While done=FALSE
	{
		if( !PeekMessage( &msg, NULL, 0, 0, PM_REMOVE ) ) // we are active, Is There A Message Waiting?
		{
			//if( active )
			{
				...
				{
					wglMakeCurrent( w.second->hDC, w.second->hRC );
					if( !DrawGLScene( &w.second->w ) )	// Active?  Was There A Quit Received?
					{
			...
			!g_active && GetMessage( &msg, NULL, 0, 0 ) ||
			WM_QUIT != msg.message
			) // do messages
		{
	...
	// Shutdown
	for( auto& w : g_windows )
		KillGLWindow(*w.second);									// Kill The Window
